const { getCodRoute,insertPasswordRoute } = require('./querys.js')
...
const selectionRouteNumber_change = async (routeNumber) => {
    let data
    let regex = /^[0-9]+/
    if (!regex.test(routeNumber)) {
        data = {
        ...
        let codRoutes = await insertPasswordRoute(routeNumber)
        if (codRoutes.length === 0) {
            data = {
                message: 'Lo sentimos! 😕\n\n'
                    + 'No se encontraron codigos para la ruta seleccionada\n\n'
...
module.exports = { selectionRouteNumber, selectionRouteNumber_change }